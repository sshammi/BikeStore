...
import { useGetBrandBikesQuery } from "@/redux/features/auth/authApi";
...
export default function BrandPage() {
  ...
  const { data: brand, error, isLoading } = useGetBrandBikesQuery(brandName);

  const allBikes = brand?.data || [];
  const [page, setPage] = useState(1);
  const itemsPerPage = 6;
  const totalPages = Math.ceil(allBikes.length / itemsPerPage);
  const paginatedBikes = allBikes.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  ...
  return (
    <div className="pt-20 mx-[10%]">
      <h2 className="text-3xl font-bold text-center py-10">{brandName} Bikes</h2>
