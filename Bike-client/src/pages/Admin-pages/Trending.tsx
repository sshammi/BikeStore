...
import { useGetAllBikesQuery, useDeleteBikeMutation, useGetFlashBikesQuery, useGetTrendingBikesQuery, useUpdateBikeMutation } from "@/redux/features/auth/authApi";
...
const TrendingProducts = () => {
  ...
  const { data: bikesResponse, isLoading, isError } = useGetTrendingBikesQuery({});
  ...
  const handleEdit = async (id: string) => {
    const confirm = window.confirm('Are you sure you want to remove this product from trending?');
    if (!confirm) return;
    try {
      const updateData = {
        id,
      ...
      toast.success('Trending status removed');
    } catch (error) {
