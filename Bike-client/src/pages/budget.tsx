...
export default function BudgetPage() {
  const { budgetRange } = useParams<{ budgetRange: string }>();
  const navigate = useNavigate();

  // Convert "Under 50000" → 50000
  const maxPrice = budgetRange ? parseInt(budgetRange.replace(/\D/g, ""), 10) : null;

  const { data, error, isLoading } = useGetAllBikesQuery(undefined);
  const allBikes = data?.data || [];

  // Filter bikes under the budget
  const filteredBikes = maxPrice
    ? allBikes.filter((bike: TBike) => Number(bike.price) <= maxPrice)
    : [];
  ...
  const paginatedBikes = filteredBikes.slice((page - 1) * itemsPerPage, page * itemsPerPage);

  if (isLoading) return <div className="pt-20 text-center">Loading...</div>;
  if (error) return <div className="pt-20 text-center text-red-500">Error loading bikes</div>;

  return (
    <div className="pt-20 mx-[10%]">
      <h2 className="text-3xl font-bold py-10 text-center">
        Bikes Under {maxPrice ? `BDT ${maxPrice}` : "Your Budget"}
      </h2>

      {filteredBikes.length === 0 ? (
        <p className="text-center text-gray-600">
          No bikes available under BDT {maxPrice}
        </p>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {paginatedBikes.map((bike: TBike) => (
